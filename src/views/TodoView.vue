...
import { apiLogout, apiCheckToken, apiGetTodo, apiAddTodo, apiTodoChange, apiDelete, apiDeleteDone } from '../../apis/user.js'
...
export default {
  setup() {
    ...
    const checkBoxToDone = (targetId, index) => {
      ...
      //     }
      //   })
      apiTodoChange(targetId, token) // 加了apiTodoChange(targetId,'', token) 會壞掉
        .then((res) => {
        ...
        if (result.isConfirmed) {
          ...
          Promise.all(
            data.Done.map((item) => {
              ...
              //   }
              // })
              return apiDeleteDone(item, token)
            })
          ).then(() => {
    ...
    <div class="container mx-auto min-h-[100vh]">
      ...
      <section class="mx-auto w-[90%] md:mt-4 lg:w-[50%]">
        ...
        <div class="empty flex flex-col text-center" v-if="data.data.length === 0">
          ...
          <img src="/empty1.svg" class="mx-auto w-[50%]" alt="" />
        </div>
